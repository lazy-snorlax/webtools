<template>
    <div class="rounded-xl border backdrop-blur-sm">
        <div class="flex flex-col space-y-1 p-6">
            <h3 class="text-lg flex items-center gap-2">Energy Converter</h3>
        </div>
        <div class="p-6 pt-0">
            <div class="space-y-2">
...
const units: Unit[] = [
    ...
    { key: 'kj', label: 'Kilojoule (kJ)',       factor: 1_000 },
    { key: 'cal',label: 'Calorie (cal)',        factor: 4.184 },
    { key: 'kcal',label:'Kilocalorie (kcal)',   factor: 4_184 },
    { key: 'wh', label: 'Watt‑hour (Wh)',       factor: 3_600 },
    { key: 'kwh',label: 'Kilowatt‑hour (kWh)',  factor: 3_600_000 }
]

const inputValue = ref<number>(1)
const fromUnit = ref<string>('kj')
const toUnit = ref<string>('j')
