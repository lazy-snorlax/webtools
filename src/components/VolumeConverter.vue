<template>
    <div class="rounded-xl border backdrop-blur-sm">
        <div class="flex flex-col space-y-1 p-6">
            <h3 class="text-lg flex items-center gap-2">Volume Converter</h3>
        </div>
        <div class="p-6 pt-0">
            <div class="space-y-2">
...
const units: Unit[] = [
    ...
    { key: 'm',  label: 'Cubic Meters', factor: 1000 },

    { key: 'galUS',  label: 'Gallon (US)', factor: 3.78541 },
    { key: 'galUK',  label: 'Gallon (UK)', factor: 4.54609 },
]

const inputValue = ref<number>(1)
const fromUnit = ref<string>('m')
const toUnit = ref<string>('L')
...
const formattedResult = computed(() => {
  ...
  return `<span class="text-cyan-400 font-mono">${inputValue.value}</span> <span class="text-zinc-400 mx-2">${(fromUnit.value == 'm') ? fromUnit.value + '<sup>3</sup>' : fromUnit.value }</span><span class="text-zinc-500 mx-2"> = </span><span class="text-green-400 font-mono">${Number(r)}</span> <span class="text-zinc-400 mx-2">${(toUnit.value == 'm') ? toUnit.value + '<sup>3</sup>' : toUnit.value}</span>`
})
